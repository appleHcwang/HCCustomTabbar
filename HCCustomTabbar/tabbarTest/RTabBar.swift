...
//    按钮偏移底部位置
    ...
    override init(frame: CGRect) {
        ...
        plusBtn?.setBackgroundImage(UIImage(named: "tab1_default"), for: UIControlState.normal)
        plusBtn?.setBackgroundImage(UIImage(named: "tab1_selected"), for: UIControlState.highlighted)
        
        
      //  plusBtn?.addTarget(self, action: #selector(self.plusBtnAction), for: .TouchUpInside)
...
//        由于在初始化时plusbtn 和image都是赋值过的 所以我直接使用了强制解包 如果是可空链的话很长
        self.plusBtn!.size = CGSize(width:self.plusBtn!.currentBackgroundImage!.size.width , height: self.plusBtn!.currentBackgroundImage!.size.height)
 
        
        title?.centerX = self.plusBtn!.centerX
      //  title?.centerY = self.plusBtn?.frame.maxY ?? default value
            
            //CGRectGetMaxY(self.plusBtn!.frame) + bottomOffSet
